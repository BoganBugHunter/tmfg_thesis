...
numberOfElementsAlong = 50;
numberOfElementsAcross = 50;
...
for i = 1:numberOfNodesAlong
    for j = 1:numberOfNodesAcross
        ...
        Marea = [ 1-Ay(i,j) Ax(i,j) ; Ay(i,j) 0.5*area_function/(initialPassageWidth/2)-Ax(i,j) ];
        Mturning = [ (nozzleRadiusOfCurvature/Ax(i,j))*sin(Ax(i,j)*nozzleTurningAngle/NozzleLength)...
        ...
        outputVector = Mturning*Marea*localVector;
...
for j = 1:numberOfNodesAcross-1
    for i = 1:numberOfNodesAlong-1
        streamwiseVector = [ Bx(i+1,j)-Bx(i,j) , By(i+1,j) - By(i,j) ]';
        spanwiseVector = [ Bx(i,j+1) - Bx(i,j) , By(i,j+1) - By(i,j) ]';
        ...
        BxComponents(i,j) = streamwiseVector(1);
        ByComponents(i,j) = streamwiseVector(2);
    end
end
...
for j = 1:numberOfNodesAcross
    for i = 1:numberOfNodesAlong
        test = BpressureCorrections(i,13); %should it always be the middle streamline?
...
for j = 1:numberOfNodesAcross
    for i = 1:numberOfNodesAlong
        BpressuresRevised(i,j) = Bpressures(i,j) + BpressureCorrectionsNormalised(i,j);
    end
end
