...
r0 = 1.225; %[kg/m3]
p0 = 4.326e6; %[Pascals] inlet total pressure from old thesis
T0 = 
...
AreaAtThroat = 0.015;
AreaAtEnds = 0.04;
rearLoadingFactor = 0.01;
nozzleTurningAngle = 73.969/360*2*pi;
...
for i = 1:countAlong
    for j = 1:countAcross
        ...
        MthroatLocation = [ 1 + 4*rearLoadingFactor/initialStreamlineLength - meshX(i,j)*4*rearLoadingFactor/initialStreamlineLength^2 0 ; 0 1  ];
...
areaRatios = zeros(size(x,1),size(x,2));
for j = 1:countAcross
    [speeds(:,j),pressures(:,j),temperatures(:,j),densities(:,j),machNumbers(:,j),speedsOfSound(:,j),areaRatios(:,j)]...
        = streamlineSolverVer01(widths(:,j),massFlowRates(1,j));
end
...
machNumbersRevised = zeros(size(x,1),size(x,2));
for j = 1:countAcross
    for i = 1:countAlong
        machNumbersRevised(i,j) = sqrt( (2/(g-1)) * ( (pressuresRevised(i,j)/p0)^((1-g)/g) - 1 ) );
    end
end
...
hold on
axis equal
contourf(x,y,machNumbersRevised,200, 'LineColor', 'none', 'ShowText','off');
contour(x,y,machNumbersRevised,[0,0.5,1.0,1.5], 'LineColor', 'k', 'ShowText','on');
