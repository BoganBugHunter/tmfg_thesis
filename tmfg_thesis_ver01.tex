...
^{\frac{\gamma}{1-\gamma}}
\end{equation}
where $x=p_{0c}$. $T_{0c}$ is assumed to be $300 K$ as the present study assumes adiabatic conditions. 
...
\begin{figure}[H]
	...
	\begin{subfigure}{.4\textwidth}
		...
		\includegraphics[width=\linewidth]{figs/mach_legend_sch_horizontal.png}
	\end{subfigure}
	\caption{Mach contours near single cooling hole for Trent XWB film cooling study at $\frac{p_{01}}{p_2}=1.65$}
...
\begin{figure}[H]
      ...
      \caption{Coolant capacity percentage delta as a function of coolant isentropic capacity percentage delta}
...
This chapter's conclusions are in good agreement with those of the other chapters, in particular where the relationship between flow capacity and effective area is concerned. This study advocates for ongoing investigation into the reasons for which effective area varies throughout the nozzle guide vane passage. Chapter~\ref{chapter_introduction} has outlined the importance of flow capacity as the dependent variable. Chapter~\ref{chapter_geometric_throat_area} has introduced the concept of effective area, and investigated the effects if it should vary with the shape of the NGV as a result of the first stage of manufacturing, namely casting. Chapter~\ref{chapter_trailing_edge} has introduced the geometry of the NGV trailing edge, and investigated the effects if it should vary due to machining or due to in-service erosion. Finally Chapter~\ref{chapter_leading_edge} has introduced coolant not into the low-velocity wake region but into the high-velocity suction side as a coolant film, and discussed the effects of varying the injection location. In combination these chapters seek to present a holistic discussion of the main challenges to our understanding and prediction of nozzle guide vane flow capacity and loss.
