...
--a discussion of what "throat area" is:

 --how "throat area" just means the smallest line or surface that can be drawn in a nozzle - not sympathetic to anything actually happening in the flow. It only really works in 1D.
 		--need to introduce the idea of area function along the length of a streamline, so each streamline can have its minimum area in a different place.
 		--for a 1D supersonic nozzle, it is possible to solve for the flow conditions along the streamline provided area is specified as a function of position. The nozzle's mass flow 		rate capacity is a function of only the flow's minimum area, as in equation \ref{capacity_definition}.
 		--If a 2D flow is modelled as a group of adjacent streamlines, separate streamlines can have dissimilar area functions. They can have their minimum areas at points that do not 		lie along a straight line across the geometric throat.
 		--If the flow is isentropic, each streamline will experience sonic conditions at its point of minimum area, combining to form the sonic line.
 		--The effective throat area of a 2D nozzle is the sum of its streamlines' throat areas. This is distinct from the sonic line length, and may be expressed as
 		\begin{equation}
 			\int_{1}^2 \vu*{v} \vdot \vu*{r} dL
 		\end{equation}
 		as illustrated in Figure~\ref{fig:illustration_of_equivalent_throat_area_integral}.
 		\begin{figure}[H]
      		\centering
 ...
 --The flow must be thought of as a series of streamlines, where each streamline still has a minimum area somewhere along its length, just not where the ruler says it is. 
