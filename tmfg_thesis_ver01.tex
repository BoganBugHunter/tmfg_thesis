...
This section will consider the effects of varying the location of a single row of film cooling holes on the suction side of an otherwise uncooled nozzle guide vane. The introduction of film coolant in this region is shown to affect the NGV flow capacity, and this effect is shown to be sensitive to the precise location of the film cooling hole row. This section's results are compared to those of existing analytical and 2D CFD studies of the effects of film cooling on flow capacity.
...
The setting of the correct total pressure on the film cooling hole plenum has been the subject of study. Thomas Povey~\cite{povey_film_cooling_model} produced an analytical model of the effect of film cooling on flow capacity, part of which provides a mapping between the plenum total pressure and temperature, the local static pressure, and the resulting mass flow rate of coolant from the hole in question. The model is based on the assignment of nominal values to 4 parameters, these being the coolant total pressure loss coefficient,
\begin{equation}
\zeta_p = 
\frac{
	p_{0cp} - p_{0c}
...
\end{equation}
and the design ratio of plenum total pressure to mainstream total pressure,
\begin{equation}
r_{cp} =
\frac{p_{0cp}}{p_{01}}
\end{equation}
where $p_{0cp}$ and $T_{0cp}$ are the total pressure and temperature at the coolant plenum boundary, $p_{0c}$ and $T_{0c}$ are the coolant total pressure and temperature at the hole exit, $p_{01}$ and $T_{01}$ are the mainstream total pressure and temperature, $p_w$ is the local external static pressure, and $T_w$ is the local wall temperature. Assuming that the mainstream total pressure remains approximately constant up to the introduction of film coolant, the above definitions combine to express the coolant total pressure as
\begin{equation}
p_{0c} =
p_{01}
\left[
...
\right]
\end{equation}
and the coolant total temperature as
\begin{equation}
T_{0c} = 
T_{0cp}
\left[
...
\right]
\end{equation}
Having thus obtained $p_{0c}$, $T_{0c}$, the mass flow rate out of the coolant hole may be computed if the local static pressure $p_w$ is obtained by CFD or other means. Conversely this model provides a means of setting the total pressure on the plenum boundary for a given design mass flow rate of coolant and a known vane surface static pressure distribution. Nominal values assumed by the author, and used by the present study, are given in Table~\ref{film_cooling_model_nominal_values}.

\begin{table}[H]
\caption{Nominal values from film cooling model by Povey}
\label{film_cooling_model_nominal_values}
\begin{center}
\begin{tabular}{|c|c|}
\hline
Parameter & Value\\
\hline
Coolant total pressure loss coefficient $\zeta_p$ & 0.7\\
Cooling efficiency $\eta$ & 0.8\\
Metal effectiveness $\epsilon_M$ & 0.6\\
Plenum/mainstream total pressure ratio $\frac{p_{0cp}}{p_{01}}$ & 1.025\\
\hline
\end{tabular}
\end{center}
\end{table}
